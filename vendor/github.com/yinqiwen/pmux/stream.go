...
func (s *Stream) updateRemoteSendWindow() error {
	...
	//logger.Printf("####[%d]deltaWindow %d", s.ID(), delta)
	return nil
}
...
func (s *Stream) incrSendWindow(frame Frame) error {
	...
	//logger.Printf("####[%d]update send window to %d", s.ID(), s.sendWindow)
	return nil
}
...
func (s *Stream) offerData(data []byte) error {
	...
	//logger.Printf("####%s %d stream", string(data), len(data))
	return nil
}
...
START:
	...
	if window == 0 {
		logger.Printf("[%d]send window is ZERO", s.ID())
		goto WAIT
	}
