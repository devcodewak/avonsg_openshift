...
func Resolve(network string, addr string) (*net.TCPAddr, error) {
	...
	if err != nil {
		logger.Printf("Error returning a copy of the network connection: %v", err)
		return nil, err
	}

	setQueryTimeouts(fileConn)

	//logger.Printf("performing dns lookup...!!")
	result, err := DnsLookup(host, fileConn)
	if err != nil {
		logger.Printf("Error doing DNS resolution: %v", err)
		return nil, err
	}
	ipAddr, err := result.PickRandomIP()
	if err != nil {
		logger.Printf("No IP address available: %v", err)
		return nil, err
	}
	return &net.TCPAddr{IP: ipAddr, Port: port}, nil
}
...
func dialContext(ctx context.Context, network, raddr string, options *NetOptions) (net.Conn, error) {
	...
	if err != nil {
		logger.Printf("###Split %v error:%v", raddr, err)
		return nil, err
	}
	...
	IPAddr := net.ParseIP(host)
	if IPAddr == nil {
		logger.Printf("Couldn't parse IP address %v while port:%d", host, port)
		return nil, err
	}
	conn.ip = IPAddr
	...
	if err != nil {
		logger.Printf("Could not create socket: %v", err)
		if socketFd > 0 {
	...
	if nil != options {
		if isTCP && options.ReusePort {
			if nil != err {
				logger.Printf("SO_REUSEADDR fail: %v", err)
				syscall.Close(socketFd)
				return nil, err
			}
			err = reuseTCPAddr(conn)
			if nil != err {
				logger.Printf("reuseTCPAddr fail: %v", err)
				return nil, err
			}
		}
		if isTCP && len(options.LocalAddr) > 0 {
			...
			if nil != err {
				logger.Printf("ResolveTCPAddr %v fail: %v", options.LocalAddr, err)
				syscall.Close(socketFd)
				return nil, err
			}
	...
	if err != nil {
		logger.Printf("Could not connect to %s socket: %v %v", raddr, err, options)
		return nil, err
	}
	...
	err = conn.convert()
	if err != nil {
		logger.Printf("Error converting protected connection: %v", err)
		return nil, err
	}
...
func ListenTCP(laddr *net.TCPAddr, options *NetOptions) (net.Listener, error) {
	...
	if err != nil {
		logger.Printf("Could not create tcp socket: %v", err)
		if socketFd > 0 {
			syscall.Close(socketFd)
		}
		return nil, err
	...
	if nil != options {
		if options.ReusePort {
			err = reuseTCPAddr(conn)
			if err != nil {
				logger.Printf("Could not reuse tcp socket: %v", err)
				if socketFd > 0 {
					syscall.Close(socketFd)
				}
				return nil, err
	...
	if len(laddr.IP) > 0 {
		...
		if err != nil {
			logger.Printf("Could not bind listen tcp socket: %v", err)
			return nil, err
		}
	}
	...
	if nil != err {
		logger.Printf("Could not listen tcp socket: %v", err)
		return nil, err
	}
	...
	if err != nil {
		logger.Printf("Error converting protected listener: %v", err)
		return nil, err
	}
	return lis, nil
}
...
func SplitHostPort(addr string) (string, int, error) {
	...
	if err != nil {
		logger.Printf("Could not split network address: %v", err)
		return "", 0, err
	}
	port, err := strconv.Atoi(sPort)
	if err != nil {
		logger.Printf("No port number found %v", err)
		return "", 0, err
	}
...
func ListenUDP(network string, laddr *net.UDPAddr) (net.PacketConn, error) {
	...
	if err != nil {
		logger.Printf("Could not create udp socket: %v", err)
		if socketFd > 0 {
			syscall.Close(socketFd)
		}
		return nil, err
	...
	if err != nil {
		logger.Printf("Could not listen udp socket: %v", err)
		return nil, err
	}
...
func DialUDP(network string, laddr, raddr *net.UDPAddr) (net.PacketConn, error) {
	...
	err = conn.connectSocket(timeout)
	if err != nil {
		logger.Printf("Could not connect to %s socket: %v", raddr, err)
		return nil, err
	}
