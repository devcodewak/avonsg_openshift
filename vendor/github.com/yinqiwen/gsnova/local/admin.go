...
func startAdminServer() {
	...
	if len(GConf.Admin.ConfigDir) == 0 {
		logger.Printf("[WARN]The ConfigDir's Dir is empty, use current dir instead")
...
func syncConfigFile(addr string, localDir, remoteDir string, fileName string) (bool, error) {
	...
	if err == nil {
		...
		if lastModifiedHeader.Before(localFileModTime) {
			//logger.Printf("Config file:%v is not update.", localFilePath)
			return false, nil
		}
	}
	...
	logger.Printf("Persistent config:%s.", localDir+"/"+filePath)
	return true, nil
}
...
func SyncConfig(addr string, localDir string) (bool, error) {
	...
	if nil != err {
		logger.Printf("Error %v with local ip:%v", err, helper.GetLocalIPv4())
		return false, err
	}
	data, _ := ioutil.ReadAll(resp.Body)
	var confList []string
	...
	for _, conf := range confList {
		...
		for _, file := range files {
			...
			if u {
				logger.Printf("Synced config:%s success", conf)
			}
		}
	}
	return update, nil
